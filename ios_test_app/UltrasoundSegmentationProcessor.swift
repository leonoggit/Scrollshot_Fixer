...
class UltrasoundSegmentationProcessor: ObservableObject {
    ...
    init() {
        do {
            self.segmentationModel = try CAMUSSegmentationModel()
        } catch {
            self.segmentationModel = try! CAMUSSegmentationModel() // This should be handled better in production
            self.errorMessage = "Failed to initialize segmentation model: \(error.localizedDescription)"
        }
    }
    ...
    private func performVideoSegmentation(url: URL) async {
        ...
        do {
            ...
            guard let videoTrack = tracks.first(where: { track in
                Task {
                    return try await track.load(.mediaType) == .video
                }
                return false
            }) else {
